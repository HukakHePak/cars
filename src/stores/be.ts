...
export class Backend {
  ...
  static createCar(car: CarView): Promise<unknown> {
    const { idcomplectation, idengine, idkpp, iddrive, vin, price, color, prod_date: prodDate } = car
  ...
  static async getCarInfo(id: number): Promise<Car> {
    return pipi.execute("get_car_info", [id]).then((list: CarView[]) => Car.fromView(list[0]))
  }
  ...
  static async getCarsByFilter(params: CarFilter = {}): Promise<Car[]> {
    ...
    return pipi
      .execute("get_cars_by_filter", [id, sold, brand, model, fuel, kpp, drive, complectation])
      .then((list: CarView[]) => list.map((item: CarView) => Car.fromView(item)))
  }

  static async getPublicCars(): Promise<Car[]> {
    return pipi
      .public("get_public_cars", [[null, null, null, null, null, null, null, null, null, null, null, null, null, null]])
      .then((list: CarView[]) => list.map((item: CarView) => Car.fromView(item)))
  }

  static async getComplectationsByModel(idModel: number): Promise<Complectation[]> {
    return pipi
      .execute("get_complectations_by_model", [idModel])
      .then((list: ComplectationView[]) => list.map((item: ComplectationView) => Complectation.fromView(item)))
  }
  ...
  static getEnginesByFilter(): Promise<Engine[]> {
    return <Promise<Engine[]>>pipi.execute("get_engines_by_filter", [null, null, null, null, null, null, null, null])
  }
  ...
  static getUsers(id?: number): Promise<UserInfo<"iduser">> | Promise<UserInfo<"iduser">[]> {
    if (id !== undefined) {
